...
class DownloadManga {
    ...
    constructor(vidData: mangaData, data: extensionInfo, success: Function, error: Function, episodes: extensionInfoEpisode[], pause: boolean) {
        ...
        const self: DownloadManga = this;

        this.base64Image = "";
        fetch(this.metaData.image).then(x => x.blob()).then(blob => blobToBase64(blob)).then((img: string) => {
    ...
    getDirectory(baseDir: DirectoryEntry, targetDir: string, options = {}): Promise<DirectoryEntry> {
        return new Promise((resolve, reject) => {
            baseDir.getDirectory(targetDir, options, function (targetDirEntry: DirectoryEntry) {
    ...
    getDataDirectory(): Promise<DirectoryEntry> {
        return new Promise((resolve, reject) => {
            // @ts-ignore
    ...
    getFile(targetDir: DirectoryEntry, fileName: string, options = {}): Promise<FileEntry> {
        return new Promise((resolve, reject) => {
    ...
    async ini() {
        const self = this as DownloadManga;
    ...
    updateMetaData(self: DownloadManga) {
    ...
    updateDownloadStatus(self: DownloadManga) {
    ...
    downloadFileTransfer(filename: string, uri: string, self: DownloadManga, headers: any = {}): Promise<string> {
        return new Promise(function (resolve, reject) {
    ...
    readFile(self: DownloadManga): Promise<string> {
        return new Promise(function (resolve, reject) {
    ...
    async startDownload(self: DownloadManga) {
        const localMapping: {
            [key: string]: {
                "downloaded": boolean,
                "uri": string
    ...
    errorHandler(self: DownloadManga, x: string) {
        if (self.controller) {
