class AnilistHelper {
    static anilistStatus = ["CURRENT", "PLANNING", "COMPLETED", "DROPPED", "PAUSED", "REPEATING"];
    static async makeAnilistReq(query: string, variables: any, accessToken: string, showAlert = true) {
        try {
    ...
    static async getAvatar(): Promise<string | undefined> {
    ...
    static async updateEpWatched(anilistID: any, epNum: any) {
    ...
    static async deleteAnilistShow(anilistID: any) {
        try {
            const accessToken = localStorage.getItem("anilist-token");
            ...
            const listID = JSON.parse(await AnilistHelper.makeAnilistReq(query, getIDvariables, accessToken))?.data?.Media?.mediaListEntry?.id;
    ...
    static addRoom(roomName: string): Promise<any> {
    ...
    static async addShowToLib(data: { name: string, img: string, url: string, currentURL: string, currentEp: number, categoryID: number }) {
        try {
            await apiCall("POST", {
                "username": "",
                "action": 5,
    ...
    static async updateAnilistStatus(aniID: any) {
        const statuses = AnilistHelper.anilistStatus as anilistStatus[];
        let promptObj = [];
        ...
        if (status) {
            try {
                ...
                await(window.parent as cordovaWindow).AnilistHelperFunctions.changeShowStatus(aniID, status);
    ...
    static async changeShowStatus(anilistID: any, status: anilistStatus) {
        const accessToken = localStorage.getItem("anilist-token");
    ...
    static async infoPromise(showURL: string, engine: extension, index: number) {
    ...
    static async batchPromises(showURLs: { showURL: string, engine: extension }[], batchSize: number, links: AnilistLinks, permNoti: notification) {
    ...
    static async malsyncPromise(url: string, id: number) {
    ...
    static async malsyncApiPromise(type: string, id: number, aniToMal: Object) {
        let malId: string;

        if (id.toString() in aniToMal) {
            malId = aniToMal[id.toString()];
        } else {
            malId = await(window.parent as cordovaWindow).anilistToMal(id.toString(), type.toUpperCase() as "ANIME" | "MANGA");
        }
    ...
    static async batchPromisesMalSyncApi(URLs: { id: number, url: string, type: string }[], batchSize: number, anilistData: { [key: number]: string }, permNoti: notification) {
        const allSettled = "allSettled" in Promise;
    ...
    static async batchPromisesMalSync(URLs: { id: number, url: string }[], batchSize: number, anilistData: { [key: number]: string }, permNoti: notification) {
        const allSettled = "allSettled" in Promise;
    ...
    static async getAllItems(auto = false) {
        ...
        try {
            const userData = await AnilistHelper.getUserData() as any;
            for (let typeIndex = 0; typeIndex <= 1; typeIndex++) {
                ...
                if (type === "manga") {
                    ...
                    const selectedValue = supportedAnimeValues.includes(defaultValue) ? 
                                          defaultValue.toString() :
                                          await thisWindow.Dialogs.prompt(
                                                "Select the main source (9anime is unstable, so it's highly recommended to not use 9anime)",
                                                "3",
                                                "select",
